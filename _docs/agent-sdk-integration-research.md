...
| **Sub-agents** | 通过 `agents` 配置传入自定义定义 + Task tool 调用，**不支持嵌套** |
...
- 子 agent **不能嵌套**——不要在子 agent 的 `tools` 中包含 `Task`
- MCP 工具在 query 级别配置，子 agent 通过 `tools` 数组引用 MCP 工具名（如 `mcp__server__action`）
- 子 agent 的 `AgentDefinition` 没有 `mcpServers` 字段
...
9. **Sub-agent 的 AgentDefinition 没有 mcpServers 字段**——MCP 在 query 级别配置，子 agent 通过 tools 数组引用
