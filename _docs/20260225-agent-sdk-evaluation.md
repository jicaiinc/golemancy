...
**核心差异（源码分析修正后）**：两者在 per-agent 配置粒度上已趋近一致（SDK v0.2.52 支持 per-agent Skills/MCP/Tools）。**唯一根本冲突是嵌套模型**：Golemancy 无限递归 vs SDK 单层。其次是 Model Provider 锁定（仅 Anthropic）和生命周期模型差异。
...
| **Sub-Agent 嵌套** | ✅ 无限层级（lazy-load 递归 + DI） | ❌ 仅单层（CLI `iP6` 硬编码限制） | 🔴 根本不同 |
| **Sub-Agent 独立性** | ✅ 完整独立（model/skills/MCP/tools/sub-agents） | ~~⚠️ 仅 prompt+tools+model~~ → ✅ 完整独立（除嵌套外） | ~~🔴~~ → 🟡 **仅嵌套差异** |
...
鉴于 SDK v0.2.52 的实际能力远超此前基于文档的评估，建议修正为：

1. **结论需要修正**：设计理念差异从 "根本冲突" 降级为 "局部差异"（仅嵌套限制）
2. **重新评估集成价值**：如果 Golemancy 的使用场景中无限嵌套不是高频需求，集成的代价-收益比需要重新计算
3. **持续关注 SDK 演进**：AgentDefinition 的快速扩展表明 Anthropic 在主动增强 per-agent 能力，嵌套限制可能在未来版本放开
4. **建议进行实际功能验证**：本分析基于源码静态分析，建议编写实际测试代码验证 `mcpServers` 和 `skills` 字段在 sub-agent 中的运行效果
