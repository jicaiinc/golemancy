...
在 UI-First 和 Server 实现阶段之后，本阶段为 Golemancy 搭建了 **端到端（E2E）测试体系** —— 基于 Playwright 的 Electron 实验性 API，直接启动编译后的 Electron 应用、自动 fork Server 子进程，验证从 UI 到 Server 的完整用户流程。
...
| **数据隔离** | 临时目录 + `GOLEMANCY_DATA_DIR` 环境变量，与真实用户数据完全隔离 |
| **API Key 管理** | `.env.e2e.local`（gitignored）→ global-setup 种子化 → Server 自动读取 |
| **双轨验证** | Store 状态断言（可靠）+ DOM 可见性断言（用户视角） |
| **生产代码改动** | 仅 2 处 env var 兼容（`GOLEMANCY_ROOT_DIR` / `GOLEMANCY_FORK_EXEC_PATH`），向后兼容 |

### 核心设计原则

- **真实运行**：不 Mock Server，不 Mock 数据库 —— Electron 真实启动、Server 真实 fork、SQLite 真实读写
- **测试隔离**：每次运行使用 `os.tmpdir()/golemancy-e2e-{pid}` 作为数据目录，互不干扰
...
| 状态桥接 | **window.__GOLEMANCY_STORE__** | — | Zustand Store 暴露给测试 |
...
| 文件 | 变更 |
|------|------|
| `apps/desktop/src/main/index.ts` | `GOLEMANCY_ROOT_DIR` + `GOLEMANCY_FORK_EXEC_PATH` env var 兼容 |
| `packages/ui/src/stores/useAppStore.ts` | `window.__GOLEMANCY_STORE__` 暴露（非 production） |
...
const app = await _electron.launch({
  ...
  env: {
    ...process.env,
    GOLEMANCY_DATA_DIR: testDataDir,          // 临时数据目录
    GOLEMANCY_FORK_EXEC_PATH: nodePath,       // node 绝对路径
    GOLEMANCY_ROOT_DIR: ROOT_DIR,             // monorepo 根路径
    NODE_ENV: 'test',
  },
  timeout: 30_000,
})
...
```
测试代码 ──evaluate()──→ window.__GOLEMANCY_STORE__
                         │
                         ├── getState()      完整状态快照
...
│
├── 2. 创建临时目录                  os.tmpdir()/golemancy-e2e-{pid}/
│
...
│      }
│
├── 4. 存入 process.env              GOLEMANCY_TEST_DATA_DIR = 临时路径
│
└── 5. Fixture 传给 Electron         env: { GOLEMANCY_DATA_DIR: testDataDir }
       │
...
globalSetup → settings.json (临时目录)
  ...
  └── defaultProvider: 'google'
       │
       ▼
GOLEMANCY_DATA_DIR → Server 读取
  │
  └── FileSettingsStorage.get() → resolveModel() → API 调用
```

**零生产代码改动**：Server 的 `FileSettingsStorage` 本来就从 `GOLEMANCY_DATA_DIR/settings.json` 读取配置，E2E 只是通过 env var 指向临时目录。
...
| | store bridge is available | `window.__GOLEMANCY_STORE__` 可访问 |
...
- `GOLEMANCY_ROOT_DIR`：解决 Playwright 启动时 `app.getAppPath()` 返回 `out/main/` 的问题
- `GOLEMANCY_FORK_EXEC_PATH`：解决 macOS GUI 进程不继承 shell PATH 的问题
...
| 坑 | 现象 | 根因 | 修复 |
|----|------|------|------|
| macOS PATH | `spawn node ENOENT` | GUI 进程不继承 shell PATH | `GOLEMANCY_FORK_EXEC_PATH` |
| app.getAppPath | `spawn node ENOENT`（cwd 不存在） | Playwright 启动时返回 `out/main/` | `GOLEMANCY_ROOT_DIR` |
...
修复：通过 `GOLEMANCY_ROOT_DIR` env var 显式传入 monorepo 根路径。
...
| 问题 | 修复 |
|------|------|
| `pkill -f 'golemancy.*server'` 匹配过宽 | 缩窄为 `pkill -f 'packages/server/src/index\\.ts'` |
...
│      │   │   └── _electron.launch({
│      │   │         args: [DESKTOP_DIR],
│      │   │         env: { GOLEMANCY_DATA_DIR, GOLEMANCY_ROOT_DIR, ... }
│      │   │       })
│      │   │
│      │   ├── → Electron 主进程
...
| 变量 | 来源 | 用途 |
|------|------|------|
| `GOLEMANCY_DATA_DIR` | E2E Fixture | 数据目录（临时目录 vs `~/.golemancy`） |
| `GOLEMANCY_ROOT_DIR` | E2E Fixture | monorepo 根路径（绕过 app.getAppPath 问题） |
| `GOLEMANCY_FORK_EXEC_PATH` | E2E Fixture | node 绝对路径（绕过 macOS PATH 问题） |

### E2E 内部使用的 env var

| 变量 | 来源 | 用途 |
|------|------|------|
| `GOLEMANCY_TEST_DATA_DIR` | globalSetup | 在 globalSetup 和 Fixture 之间传递临时目录路径 |
