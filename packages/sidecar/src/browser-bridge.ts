...
interface PollWaiter {
  profileId: string;
  resolve: (message: NativeRequest | null) => void;
  timeout: NodeJS.Timeout;
}
...
export class BrowserBridge {
  #profiles = new Map<string, BrowserProfile>();
  #queues = new Map<string, NativeRequest[]>();
  ...
  async poll(
    profileId: string,
    snapshotOrSessionId?: BrowserPollSnapshot | string,
    timeoutMs = DEFAULT_POLL_TIMEOUT_MS,
  ): Promise<NativeRequest | null> {
    const snapshot =
      typeof snapshotOrSessionId === "string" ? { sessionId: snapshotOrSessionId } : (snapshotOrSessionId ?? {});
  ...
  #handleSessionEvent(frame: NativeRequest): void {
    if (frame.method === "session.hello" || frame.method === "session.profileInfo" || frame.method === "session.heartbeat") {
  ...
  #selectProfile(profileId?: string): BrowserProfile {
    ...
    const online = [...this.#profiles.values()]
