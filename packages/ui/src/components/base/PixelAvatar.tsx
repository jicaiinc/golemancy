...
export function PixelAvatar({ size = 'md', src, initials, status, className = '', ...props }: PixelAvatarProps) {
  ...
  return (
    <div className={`relative inline-flex items-center justify-center bg-elevated border-2 border-border-dim rounded-full overflow-hidden shadow-raised ${s.container} ${className}`} {...props}>
      {src ? (
