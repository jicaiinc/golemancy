...
export function TeamTopologyToolbar({ team, onResetLayout }: TeamTopologyToolbarProps) {
  ...
  return (
    <div className="absolute top-0 left-0 right-0 z-40 bg-surface/90 border-b-2 border-border-dim px-4 py-2 flex flex-col gap-0.5 backdrop-blur-sm">
      {/* Row 1: Back + Name + Saved + spacer + buttons */}
      ...
      {/* Row 2: Description (aligned with name) */}
      <div className="flex items-center gap-3">
        {/* Spacer to align with name (Back button + divider width) */}
        <div className="shrink-0 invisible">
          <PixelButton variant="ghost" size="sm">{t('detail.backBtn')}</PixelButton>
        </div>
        ...
        {editingDesc ? (
          <input
            autoFocus
            className="font-mono text-[11px] text-text-secondary bg-deep px-2 h-7 border-2 border-accent-blue outline-none min-w-[100px] max-w-[400px] flex-1"
            value={description}
          ...
          <button onClick={() => setEditingDesc(true)} className="font-mono text-[11px] text-text-dim hover:text-text-secondary cursor-pointer truncate max-w-[400px]">
