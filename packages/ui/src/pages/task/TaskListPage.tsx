...
export function TaskListPage() {
  ...
  const grouped = useMemo(() => {
    ...
    for (const task of tasks) {
      const group = map.get(task.conversationId) ?? []
      group.push(task)
      map.set(task.conversationId, group)
    }
    return map
  }, [tasks])
  ...
  return (
    <motion.div className="p-6" data-testid="task-list-page" {...staggerContainer} initial="initial" animate="animate">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="font-pixel text-[14px] text-text-primary" data-testid="task-list-header">{t('page.title')}</h1>
          <p className="text-[12px] text-text-secondary mt-1">
            {t('page.taskCount', { count: tasks.length })}
          </p>
        </div>
      </div>
                                ...
                                {isTaskExpanded && (
                                  ...
                                  >
                                    <div className="mt-3 pt-3 border-t-2 border-border-dim space-y-2">
                                      {task.description && (
                                        <div>
                                          <span className="font-pixel text-[8px] text-text-dim">{t('detail.description')}</span>
                                          <p className="text-[12px] text-text-secondary font-mono mt-1">{task.description}</p>
                                        </div>
                                      )}
                                      {task.blockedBy.length > 0 && (
                                        <div>
                                          <span className="font-pixel text-[8px] text-text-dim">{t('detail.blockedBy')}</span>
                                      ...
                                      {task.metadata && Object.keys(task.metadata).length > 0 && (
                                        <div>
                                          <span className="font-pixel text-[8px] text-text-dim">{t('detail.metadata')}</span>
