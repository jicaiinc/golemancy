...
export type McpClientHandle = {
  ...
  readonly listTools: () => Promise<ReadonlyArray<{ name: string; description?: string; jsonSchema: unknown }>>;
  readonly callTool: (name: string, input: unknown) => Promise<unknown>;
  readonly stop: () => Promise<void>;
};

export interface McpClientFactory {
  connectStdio(spec: { id: string; command: string; args: ReadonlyArray<string> }): Promise<McpClientHandle>;
