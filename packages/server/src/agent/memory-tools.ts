...
export function buildMemoryInstructions(data: {
  ...
  lines.push(`- Not loaded: ${notLoaded} (use MemorySearch to access)`)
  ...
  lines.push('You have 4 memory tools: MemorySave, MemorySearch, MemoryUpdate, MemoryDelete.')
  lines.push('Priority scale: 0 (Archive) → 3 (Normal, default) → 5 (Critical).')
  lines.push('Higher priority memories are auto-loaded first within the limit.')
  lines.push('You can adjust priority to control which memories stay in your active context.')
  lines.push('Pinned memories are controlled by the user and always loaded regardless of limit.')
