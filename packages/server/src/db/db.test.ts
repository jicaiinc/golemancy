...
describe('database migration', () => {
  ...
  function insertConversation(id = 'conv-1', executionAgentId = 'agent-1') {
    db.run(sql`INSERT INTO conversations (
      id, execution_agent_id, target_kind, target_id, title, created_at, updated_at
    ) VALUES (
      ${id}, ${executionAgentId}, 'agent', ${executionAgentId}, 'Test', '2024-01-01', '2024-01-01'
    )`)
  }
  ...
  describe('indexes', () => {
    it('creates conversation execution agent index', () => {
      const rows = db.all<any>(sql`SELECT name FROM sqlite_master WHERE type='index' AND name='idx_conversations_execution_agent'`)
      expect(rows).toHaveLength(1)
    })
