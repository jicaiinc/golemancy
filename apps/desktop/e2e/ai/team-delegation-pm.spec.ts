...
test.describe('PM Team Delegation', () => {
  ...
  test('PM delegates to two members and returns both member outputs', async ({ helper }) => {
    ...
    expect(lowerResponse).toContain('analyst:')
    expect(lowerResponse).toContain('executor:')
    expect(lowerResponse).toContain(analystOutput.toLowerCase())
    expect(lowerResponse).toContain(executorOutput.toLowerCase())
  })
})
